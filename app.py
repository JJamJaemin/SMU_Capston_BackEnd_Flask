...
month_count_response = api.model('MonthCountResponse', {
    ...
    "eventCount": fields.String("배열 형식[2,1,2,0,0,0]", description='한달치 사건 감정 카운트 배열 neutral sad angry happy anxiety embarrassed hurt'),
    "month_feedback": fields.String("이번달의 감정변화가~~~~", description='eventCount의 감정을 비율화해서 피드백'),
...
class CountMonthemotion(Resource):
    ...
    def post(self):
        ...
        if userid is not None and month is not None:
            ...
            print("한달 내 (텍스트, 음성, 최종감정, 사건감정) 갯수 ",response)
            return response
        else:
            response = "인자값 오류"
            return response
